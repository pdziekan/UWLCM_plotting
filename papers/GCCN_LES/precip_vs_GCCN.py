...
varlabels = [ "cloud droplet concentration = $38\,  \mathrm{cm}^{-3}$" \
             ,"cloud droplet concentration = $59\,  \mathrm{cm}^{-3}$" \
             ,"cloud droplet concentration = $115\, \mathrm{cm}^{-3}$" \
            ]
CCN_conc = [95, 190, 475]
nc = [38, 59, 115]
#ensemble size
...
#  print(profs_file_names[it])
  ...
  print('tot acc sruf: ', tot_acc_surf_precip)
...
#  p = np.poly1d(z)
  ...
  axarr[0,0].plot(lx, test2(lx, param[0], param[1]), '--', label='$a x^b$ fit')
...
#  axarr[1,0].set_xlabel('${\mathrm{GCCN\, concentration}}\over{\mathrm{CCN\, concentration}}$')
#  axarr[1,1].set_xlabel('${\mathrm{GCCN\, concentration}}\over{\mathrm{CCN\, concentration}}$')
#  axarr[1,1].set_xlabel('GCCN concentration [cm$^{-3}$]')
  axarr[1,0].set_xlabel('GCCN concentration / CCN concentration')
  axarr[1,1].set_xlabel('GCCN concentration / CCN concentration')
...
#figure size
fig.set_size_inches(8, 6.)# 5.214)#20.75,13.74)
#distances between subplots and from bottom of the plot
fig.subplots_adjust(bottom=0.15 + (len(labels) - 2) * 0.06, wspace=0.3)#, hspace=0.25)
