...
void plot_fields(Plotter_t plotter, Plots plots, std::string type)
{
  ...
  for (int at = 0; at < n["t"]; ++at) // TODO: mark what time does it actually mean!
  {
    ...
    for (auto &plt : plots.fields)
    {
      ...
      Gnuplot gp;
//      init(gp, plotter.file + "_" + type + ".plot/" + plt + "/" + zeropad(at * n["outfreq"]) + ".pdf", 1, 1, n, 1.5, 2000. / 12800.);  // rico
      init(gp, plotter.file + "_" + type + ".plot/" + plt + "/" + zeropad(at * n["outfreq"]) + ".pdf", 1, 1, n, 1.5, 1000. / 6400.);  // dycoms
